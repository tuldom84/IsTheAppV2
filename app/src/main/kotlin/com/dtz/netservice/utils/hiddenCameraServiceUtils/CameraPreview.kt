package com.dtz.netservice.utils.hiddenCameraServiceUtils
...
import com.dtz.netservice.utils.hiddenCameraServiceUtils.config.CameraResolution
import com.dtz.netservice.utils.hiddenCameraServiceUtils.config.CameraRotation
import com.dtz.netservice.utils.hiddenCameraServiceUtils.HiddenCameraUtils.rotateBitmap
import com.dtz.netservice.utils.hiddenCameraServiceUtils.HiddenCameraUtils.saveImageFromFile
...
internal class CameraPreview(context: Context, private val mCameraCallbacks: CameraCallbacks) : SurfaceView(context), SurfaceHolder.Callback {
    ...
    override fun surfaceChanged(surfaceHolder: SurfaceHolder, i: Int, i1: Int, i2: Int) {
        ...
        Collections.sort(pictureSizes,
           com.dtz.netservice.utils.hiddenCameraServiceUtils.PictureSizeComparator()
        )

        val cameraSize: Camera.Size? = when (cameraConfig!!.resolution) {
            CameraResolution.MEDIUM_RESOLUTION -> pictureSizes[pictureSizes.size / 2]
            CameraResolution.SLOW_RESOLUTION -> pictureSizes[pictureSizes.size - 1]
        ...
        if (camera != null) {
            camera!!.takePicture(null, null, Camera.PictureCallback { bytes, _ ->
                Thread(Runnable {
                    ...
                    val rotatedBitmap: Bitmap? = if (cameraConfig!!.imageRotation != CameraRotation.ROTATION_0)
